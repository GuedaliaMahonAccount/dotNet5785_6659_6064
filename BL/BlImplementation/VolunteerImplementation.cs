...
internal class VolunteerImplementation : IVolunteer
{
    ...
    /// <summary>
    /// Updates volunteers with null CallType to inactive if their last assignment was more than 5 years ago.
    /// </summary>
    /// <exception cref="BlDoesNotExistException">
    /// Thrown when:
    /// - The volunteer or assignment does not exist in the database.
    /// - Other unexpected errors occur during the process.
    /// </exception>
    ...
    /// </exception>
    /// <summary>
    /// Removes volunteers who have not had an active call for more than 5 years.
    /// </summary>
    /// <exception cref="BlDoesNotExistException">
    /// Thrown when:
    /// - The volunteer or assignment does not exist in the database.
    /// - Other unexpected errors occur during the process.
    /// </exception>
    public void RemoveVolunteersWithNoActive()
    {
        try
        {
            var volunteersFromDal = _dal.Volunteer.ReadAll();
            var assignmentsFromDal = _dal.Assignment.ReadAll();
                    ...
                    if (volunteerAssignments.Any())
                    {
                        ...
                        if (latestAssignment.StartTime < DateTime.Now.AddYears(-5))
                        {
                            ...
                            Action observer = () =>
                            {
                                ...
                                Console.WriteLine($"Observer removed for volunteer with ID {volunteer.Id}.");
                            };

                            // Remove the observer
                            VolunteerManager.Observers.RemoveObserver(volunteer.Id, observer);
                        }
                    }
                    else
                    {
